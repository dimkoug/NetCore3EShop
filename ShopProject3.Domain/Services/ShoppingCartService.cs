...
namespace ShopProject3.Domain.Services
{
    public class ShoppingCartService : IShoppingCartService
    {
        ...
        async Task IShoppingCartService.AddToCart(string SessionCartId, int productId)
        {
            ...
            {
                shoppingCartItem = new ShoppingCartItemsEntity
                {
        ...
        async Task<List<ShoppingCartItems>> IShoppingCartService.GetShoppingCartItems(string SessionCartId)
        {
            var data = await _context.ShoppingCartItems.Include(c=>c.Product).Where(c => c.SessionCartId == SessionCartId).ToListAsync();
            var items = _mapper.Map<List<ShoppingCartItemsEntity>, List<ShoppingCartItems>>(data);
            return items.ToList();
        }
