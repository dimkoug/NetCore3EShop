...
namespace ShopProject3.MVC.Controllers
{
    public class EventsController : Controller
    {
        ...
        {
            var data = await _eventsService.GetAll();
            ...
            return View("~/Views/Events/Manage.cshtml", result);
        }
        public async Task<IActionResult> Details(int? Id)
        {
            ...
            var model = await _eventsService.Get(Id);
            if (User.IsInRole(Roles.Client) || !User.Identity.IsAuthenticated)
            {
                return View(model);
            }
            ...
            return View("~/Views/Events/ManageDetail.cshtml", model);
        }
        [Authorize]
        ...
        [Authorize]
        public async Task<IActionResult> Create(Events viewModel)
        {
            if (ModelState.IsValid)
            {
                var Id = await _eventsService.Add(viewModel);
        ...
        public async Task<IActionResult> Edit(int Id)
        {
            ...
            var viewModel = await _eventsService.Get(Id);
            return View(viewModel);
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        [Authorize]
        public async Task<IActionResult> Edit(int Id, Events viewModel)
        {
            if (Id == null)
            {
                return NotFound();
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    await _eventsService.Update(viewModel);
                    if (!String.IsNullOrEmpty(Request.Form["continue"]))
                    {
                        return RedirectToAction("Edit", new { Id = Id });
                    }
        ...
        [ValidateAntiForgeryToken]
        [Authorize]
        public async Task<IActionResult> DeleteConfirmed(int? Id, Events viewModel)
        {
            if (User.IsInRole(Roles.Client) || !User.Identity.IsAuthenticated)
            {
                return NotFound();
        ...
        {
            return await _eventsService.Exists(id);
        }
    }
}
